...
export interface UseOpponentPrepNotesResult {
...
export function useOpponentPrepNotes(username: string | null): UseOpponentPrepNotesResult {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(false)
    fetch(`${API_BASE}/api/opponent-prep/${encodeURIComponent(username)}/notes`)
      .then((r) => {
        if (!r.ok) throw new Error(`status ${r.status}`)
        return r.json() as Promise<NotesBody>
      })
      .then((body) => {
        if (cancelled) return
  ...
  function generate() {
    ...
    setGenerating(true)
    setGenerateError(null)
    fetch(`${API_BASE}/api/opponent-prep/${encodeURIComponent(username)}/notes/generate`, {
      method: 'POST',
    })
      .then(async (r) => {
