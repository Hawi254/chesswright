...
# One entry per field/section on the React Settings page (category rail +
# detail pane, docs/superpowers/specs/2026-07-17-react-settings-page-design.md).
# Each entry's url_path is a real category route under settings/*, and each
# carries an anchor matching that field's element id, so a hit deep-links
# straight to the field (scrolled to and briefly highlighted) instead of
# just the bare "settings" page. Hand-kept in sync with
# frontend/src/lib/navCandidates.ts's matching list -- same accepted drift
# risk as navConfig.ts's group bucketing.
SETTINGS_CANDIDATES = [
