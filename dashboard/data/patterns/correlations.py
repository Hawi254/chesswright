...
from .._shared import (
    THINKING_TIME_BUCKETS, TIME_PRESSURE_BUCKETS, bucket_acpl_blunder_rate, _fetchone_scalar,
)
...
def get_decisive_moments_breakdown(duck_conn):
    """Relocates the three transforms `patterns_view.py`'s `_render_tab_turning`
    currently does inline on `get_decisive_moments`'s result -- same bins/
    labels/phase-order/TIME_PRESSURE_BUCKETS, no behavior change. See
    docs/superpowers/specs/2026-07-15-patterns-tendencies-slice2-3-turning-
    points-piece-handling-design.md decision 1."""
    dm_df = get_decisive_moments(duck_conn)
    n_losses = len(dm_df)
    if n_losses == 0:
        return {
            "n_losses": 0,
    ...
    mode = dm_df.phase.mode()
    most_common_phase = mode.iloc[0] if not mode.empty else None

    bins = [0, 6, 11, 16, 21, 26, 31, 41, 60, 9999]
    labels = ["1–5", "6–10", "11–15", "16–20", "21–25",
              "26–30", "31–40", "41–60", "60+"]
    dm_df["move_bucket"] = pd.cut(dm_df.move_number, bins=bins, labels=labels, right=False)
    mn_df = dm_df.groupby("move_bucket", observed=True).size().reset_index(name="n_losses")
    by_move_bucket = [{"bucket": str(r.move_bucket), "n_losses": int(r.n_losses)}
                       for r in mn_df.itertuples()]

    phase_order = ["opening", "middlegame", "endgame"]
    ph_df = (dm_df.groupby("phase").size().reset_index(name="n_losses")
              .set_index("phase").reindex(phase_order).dropna().reset_index())
    by_phase = [{"phase": r.phase, "n_losses": int(r.n_losses)} for r in ph_df.itertuples()]

    clock_df = dm_df.dropna(subset=["clock_fraction"])
    by_clock_bucket = []
    for label, lo, hi in TIME_PRESSURE_BUCKETS:
        n = int(((clock_df.clock_fraction >= lo) & (clock_df.clock_fraction < hi)).sum())
        if n:
            by_clock_bucket.append({"bucket": label, "n_losses": n})
    n_no_clock_data = n_losses - len(clock_df)

    return {
        "n_losses": n_losses,
        "median_move": median_move,
        "most_common_phase": most_common_phase,
        "by_move_bucket": by_move_bucket,
        "by_phase": by_phase,
        "by_clock_bucket": by_clock_bucket,
        "n_no_clock_data": n_no_clock_data,
    }
