...
| Multi-select Opening Accuracy comparison | ⚠️ | Shipped as **2-opening only**, not true N-way. `theme.render_comparison_panel` hard-caps at 2 panes with no path to 3+ through it — a real, permanent ceiling of the current component, not a temporary gap. Revisit only if a genuine N-way use case appears; would need a new component, not an extension of this one. |
...
| Playing Sessions tab | ⚠️ | Shipped as a schema extension + UI on top of existing `session_ctx_cache` (`patterns_view.py`'s `_render_tab_sessions`) — but **no tournament grouping** exists. The roadmap's own line explicitly named tournament grouping as a real remaining gap; it still is. |
